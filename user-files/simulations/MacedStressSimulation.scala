  	...
  	object SubmitLoginDetails {
  		...
  		val submitLoginDetails = group("Submit Valid Login Details") {
	  		...
	  		exec(http("postLoginDetails")
				...
				.get("/index-xml.php?site_id=RDCv2&callback=jQuery183106605701891239733_1448553188598&action=get_sgk_cookie&SID=qe82igsgtngvjpqejfafl8sou1&_=1448553266077")
				.check(headerRegex("Set-Cookie","sgk_cookie=(.*)").saveAs("USERPASS"))
				.silent
				.headers(headers_cache_control),
	            http("getResourceList2")
				.get("/index-xml.php?site_id=RDCv2&callback=jQuery183106605701891239733_1448553188599&action=get_MEE_OAUTH_cookie&SID=qe82igsgtngvjpqejfafl8sou1&_=1448553266080")
				.check(headerRegex("Set-Cookie","MEC_WS_SESSIONID=(.*)").saveAs("MEC_WS_SESSIONID"))
				.silent
				.headers(headers_cache_control),
	            http("getResourceList3")
				.get("/index-xml.php?site_id=RDCv2&callback=jQuery183106605701891239733_1448553188598&action=get_MEE_OAUTH_cookie&SID=qe82igsgtngvjpqejfafl8sou1&_=1448553266210")
...
// Set up URL paths for all required resources  
	...
	setUp(scn.inject(splitUsers(10) into (rampUsers(5) over (1 seconds)) separatedBy(30 seconds))).protocols(httpProtocol)
}